...
*** TODO Stefan  "(VARS VALS STORE-VAR STORE-FORM ACCESS-FORM)" I don't follow this
...
*** TODO Stefan "NIH syndrome" That seems a different reason that "the existing
...
*** TODO Stefan "a DO function of two arguments" Is this what is called
...
*** TODO Stefan Are the object facilities of both EIEIO and cl-generic.el currently in
...
** 2019-03-29 w/ Brent Hailpern

contrast "normal language design" and Elisp design
- some part of it is usual, of course
- usually there's a language philosophy, but here it's maybe more driven by
  practical needs?
- discuss how Elisp had to adapt to changes in Emacs (e.g unicode)

Discuss how the language was designed initially from the point of view of
"what do we need to for an editor like Emacs" (key-bindings, model of
buffers, IO model, functions, commands, ...).
[ I guess that points also to the design of the interactive spec.  ]

Need a Maclisp or core Elisp appendix at least, and we may want to present
near the beginning a bit more of what Elisp looks like.

Talk more on how the switch to lexical took place.

How a language evolves when it's not in control of its destiny (because it
mostly just serves the needs of Emacs).

Maybe expand on the IO model.

[ Not discussed but crossed my mind: I think it would be nice if the
  article could make the reader understand how Elisp is a language where
  everything takes place within a large state context, how state is
  so ever present (and hence why multithreading is nigh-on impossible).  ]

Do not just refer the reader to to the Maclisp manual

See about how Lucid had different needs and how that explains the
corresponding changes in design

Investigate how early development&design was organized

Clarify "there was no lambda initially"
