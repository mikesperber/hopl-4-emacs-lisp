...
% \verb|\subsection|
% FIXME: Mike hates it, too.  Also initial sections called "Introduduction"
...
\end{itemize}
%
% FIXME: Section organizaetion

\subsection{Language design}

The base language of Emacs Lisp started out as a straightforward of
MacLisp~\ref{Moon1974,Pitman1983}.
...
\textit{property list}, essentially a dictionary mapping property
...
% FIXME: Overview over coming subsubsections
...
\textit{condition system}.  The \Elisp{} condition system establishes
...
\subsubsection{Buffer-local variables}
\label{sec:buffer-local-variables}

Buffer-local variables are the prominent feature marking \Elisp{} as
the extension language of a text editor: Calling
\texttt{make-variable-buffer-local} on the name of a global variable
makes it \textit{buffer-local}.  This associates the value of the
variable with the \textit{current buffer}, a global reference managed
by Emacs that determines the implicit target of editing operations.
An assignment to a buffer-local variable only affects dereferences
with the same current buffer in effect.  In the absence of such an
assignment, a buffer-local variable dereferences its special
\textit{default value}.

It is also possible to call \texttt{make-local-variable} on a variable
name, which makes the variable have a local variable on the buffer
current at the time of the \texttt{make-local-variable} invocation.
Moreover, Emacs features a small number of \textit{terminal-local}
variables.
...
additional code without having to change them, as as to extend their
...
Hooks are not a core language feature, but their use has been is a
...
\subsubsection{Byte-code interpreter}
...
also added \textit{ephemerons} to \Elisp{}~\cite{Hayes1997}.
