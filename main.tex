...
years, and thus its evolution did happen along a single timeline.
...
We trace the overall evolution \Elisp{} in the software projects that
implemented it. Its predecessors EINE (1976) and Multics Emacs (1978)
were themselves written in Lisp.  Unix Emacs (also known as Gosling
Emacs) (which appeared in 1981) included its own Lisp.  We briefly
describe those predecessors in Section~\ref{sec:prehistory}.

Emacs as we know it today itself started in the early 1980s.  Its
development continued at a high pace until about 1991---described in
Section~\ref{early-history}.  Around that time, it slowed down and
room for Lucid Emacs, later renamed XEmacs, whose development of
...
\end{itemize}
%
TECO and Gosling Emacs featured small languages that were too weak to
support this.  Consequently, Stallman took inspiration from MacLisp,
and \Elisp{} started as a real programming language with powerful
abstraction facilities, thus foregoing Greenspun's tenth rule.
...
available in \Elisp.
As a
result, various functions dealing with numbers beyond the fixnum range
had to implement workarounds.  Notable are \texttt{file-attributes}
(which may use a pair of two fixnums for inode numbers and device
numbers may use a float for the file size) and \texttt{current-time},
which returns a list of numbers to encode the time.

Moreovrer, Emacs Lisp was used for more and more applications beyond
...
Jerry James added bignums to XEmacs 21.5.18 in 2004.  Efforts to add
bignums to Emacs are underway at the time of writing of this article,
in 2018.

The support for bignums in XEmacs is optional at build time, and hence
XEmacs \Elisp{} programs still cannot rely on bignum support.
Consequently, \texttt{file-attributes}, \texttt{current-time} and Calc
are still as they were in the pre-bignum days.

The introduction of bignums raises some design issues in \Elisp, as
previously integers were always unboxed.  As a result, \Elisp{} could
assume that, if two integers represented the same number, \texttt{eq}
would return true on them.  Bignums are heap-allocated, so the same is
not necessarily true for two bignums.  In XEmacs, \texttt{eq} can
return \texttt{nil} in this case, and this seems to have caused no
serious problems.  This issue is still being discussed with Emacs.  % FIXME Stefan: or is it?

% FIXME Stefan: I think this is the detail to which this is
% interesting.  The same level of detail for Emacs would be nice, but
% I haven't been able to figure out the current state of the discussion.
