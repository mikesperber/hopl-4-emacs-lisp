...
%% FIXME Mike: When did it get fixed in XEmacs?
%% Mike says: I don't see that it was ever "fixed".  What's the bug
%% anyway - any behavior I can imagine seems surprising to some.
%% FIXME Mike: I explained the bug.
...
%% FIXME Mike:
%%    We say nothing about XEmacs here :-(
%%    IIRC there was a round of micro-optimisation in the
%%    XEmacs byte-code interpreter
...
still essentially the same as that of 1985.
...
modified while Emacs was running, so while this feature is always used
...
simple way, and did not include any operation beyond
...
In 2012 during the development of Emacs-24.3, the byte-code interpreter was
modified by Tom Tromey to implement token threading using GCC's
\emph{computed goto} feature when available.  Patches for this feature had
been submitted several times in the past already, but the speed improvement
was rather disappointing so they had not raised much enthusiasm.
Tom's implementation was no better, but he pushed stronger for its
inclusion.  The reason why the speed improvement is disappointing was not
really investigated, but the general consensus is that the byte-code
interpreter is simply not very optimized, so the relative cost of the
\texttt{switch} is not as high as it could (or, arguably, should) be.

\paragraph{Tail-call optimisation}

\Elisp{} does not optimize away tail calls.  With Scheme being familiar to
many \Elisp{} developers, this is a disappointment for many.  There have
%% FIXME: detail at least some concrete examples!
been patches submitted to Emacs to optimize away at least some tail calls in
byte-compiled code, but so far none of them have made it into an official
release.  The main reason for that is partly a chicken-and-eggs problem:
First tail-call optimization (TCO) is largely incompatible with dynamic
scoping, so it was basically inapplicable until the introduction of lexical
scoping in 2012, then \Elisp{} function calls are relatively expensive and
finally the lack of TCO created a coding style which favored the use of
iteration over recursive definitions, which in turn makes TCO rarely
beneficial on existing code.
...
%% FIXME: \subsection{C FFI}

%% FIXME: Variable aliases (Gerd added them to Emacs-22.1, introduced earlier in XEmacs)
...
%% FIXME: Should this go to the bytecode section?
...
of those constructs.  So in Emacs-24.4, new byte codes were introduced and
...
%% it is.
In 1999, Sam Steingold also implemented the \Elisp{} language as
a Common Lisp package~\cite{Steingold99}.  It does not attempt to
reimplement the library of functions provided in Emacs to manipulate buffers
and other related objects, so it focuses on the ``pure'' \Elisp{} language.
...
We'd like to thank Lars Brinkhoff for his archiving work at
