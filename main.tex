...
% \verb|\subsection|
% FIXME: Mike hates it, too.  Also initial sections called "Introduction"
...
\end{itemize}
%
% FIXME: Section organization

\subsection{Language design}

The base language of Emacs Lisp started out as a straightforward
implementation of MacLisp~\ref{Moon1974,Pitman1983}.
...
%% FIXME: I think we have a problem with the nesting: error-handling and
%% buffer-local vars should be subsections, not subsubsections.
...
\subsubsubsection{Terminal-local and frame-local variables}

%% FIXME Mike: Add XEmacs's view to this.  Specifiers?

Emacs calls GUI windows \emph{frames} and as with buffers, Emacs
maintains a reference to the \emph{current frame} which determines the
implicit target of GUI operations.

In 1995, Emacs-19.29 added the ability to have \emph{frames} on several
different X11 servers at the same time.  On that occasion, it also added the
notion of \emph{terminal-local} variables, which are variables which take
different values depending on the X11 server (the \emph{terminal}) to which
the current frame belongs.  The set of terminal-local variables is small and
predefined in the C code; they are mostly used internally to keep track of
things like keyboard state; there is no way for \Elisp{} programs to
create others.

Then in 1998 Emacs-20 added the ability to set a frame-local value to
a variable.  Contrary to terminal-local variables, any variable can be made
...
%% FIXME: "recently" should be made more precise.  Also IIRC
...
%% I can't remember exactly what were the reason for that hack in Cygwin)).
