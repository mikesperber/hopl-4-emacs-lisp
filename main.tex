...
\item It should be easy for the end-user to find the relevant code in order
  ...
  the language: the use of some facilities, such as \emph{advice} (see
...
Seeing how uses of old-style backquotes were not going away by themselves, in 2007
...
\end{verbatim}
%
This \emph{expression} evaluates to a nested list structure with the
...
\texttt{cl.el} package, see Section~\ref{sec:cl-lib}.  Use of
\texttt{defstruct} in \Elisp{} code only began to be commonplace around
2010.  Until 2013, structures were simply vectors, when Lars Brinkhoff added
the record data type to make structures distinct from them.
Making \texttt{defstruct} use this new data type introduced some backward
incompatibilities, so the change lived in a separate branch for four years
during which various mitigation strategies were developed, until it was
finally included in Emacs-26.1.
...
available in MacLisp and Lisp Machines. This form allows attaching code to
...
Emacs, which is why Richard Stallman was happy to include it, but he has always made
...
char of a string can be annotated with a set of properties, mapping1
...
Stallman.  The first is a very simple interpreter written in C
...
custom data type definitions for representing \Elisp{} entities  (see
...
disabled on many platforms beause of bugs in the implementation of
unions and bit fields in C compilers.  For the release of XEmacs 19.13
in 1995, Ben Wing merged the data representation from Emacs 19.30.
...
argument is expected to be a list whose first element is just a mere symbol
rather than an expression and hence should not be instrumented, whereas the
subsequent element is expected to be a \texttt{form} which means a normal
expression that can be instrumented.
...
Section~\ref{sec:structures} and
Section~\ref{sec:data-representation}.
...
multiprecision library.  The deciding factor which changed the tradeoff was
the fact that by Emacs-25, most builds of Emacs made use of the GNUtls
...
in 1986 Cesar Quiroz \email{<quiroz@cs.rochester.edu>} wrote a \texttt{cl.el} package that provided various
...
\item Some aspects of Common Lisp's design can incur 
  ...
  significant amount of memory.)  Consequently, there were good reasons to try and avoid making
...
\item[2001] Then maintainer Gerd Möllmann has a more favorable opinion of
  Common Lisp.  As a result, Emacs-21.1 includes the hash-table functions,
  reimplemented in C and extended.  It also adopts the Common Lisp concept
  ...
  are also added to \Elisp{}, although they introduced some difficulties: in
  ...
  the macros added to \Elisp{} do not actually replace those of
  \texttt{cl.el}; instead when \texttt{cl.el} is loaded, it now overrides the
...
\texttt{cl-lib.el} that provides the same facilities, but with names which
...
came before its expansion was chosen, because of the way it sounds.
...
% FIXME Stefan: Rudimentary example would be nice, as well as
% defmethod below
...
The result of this was finally released in 2018, as part of Emacs-26.1.
...
patch was finally accepted only because it was ready, it was non-invasive,
...
it, but its applicability is limited by the fact that buffers's contents
...
even more so when Stefan Monnier took over maintainership in 2008, language
design received more attention as a goal in itself.  The language designer's
perspective has driven changes such as the introduction of lexical scope,
features from Common Lisp, and \texttt{pcase}.
...
\Elisp{} is showing no signs of going away any time time.  We are
looking forward to its next 30 years.
