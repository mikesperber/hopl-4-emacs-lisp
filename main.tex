...
been two patches developped independently and submitted to Emacs maintainers
in late 2012 (by Troels Nielsen first and Chris Gay two months later) to
optimize away tail calls in byte-compiled code, but so far none of them have
...
\item They only affected byte-compiled code, and while it is expected that
  ...
  So if \Elisp{} code starts to rely on TCO, it would tend to cause problems
...
demand.  For many years, the XEmacs developers avoided changing the
byte-code format to make byte-code files interchangeable between Emacs
and XEmacs..

In late 2009, the byte-code interpreter was modified by Tom Tromey to
