...
S-expression representation for keymaps.  In Emacs 19, a keymap was
simply a two-element list whose car is the symbol \texttt{keymap} and
whose second element is a vector, indexed by character code.  This
...
MULE (Multi-Lingual Emacs).  In previous versions of XEmacs,
characters were represented by integers, and certain bits represented
modifiers such as alt, hyper, or meta.  The shift to MULE also brought
about changes in the mapping between (conceptual) characters and their
numeric codes.  As a consequence, XEmacs made the character data type
opaque and distinct in XEmacs 20.

Emacs has retained the representation of characters through numbers,
but shifted to a fixed Unicode-based encoding scheme in FIXME Stefan.
