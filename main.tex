...
% FIXME Stefan: But what was the representation, and what exactly was
% the "detriment"?
...
valid in both syntaxes but means different things: It could either
denote a backquote expression (denoting the symbol \verb|a|) or an
application of a function named \verb|`| to the value of variable
\verb|a|.

So the old format was declared obsolete,
% FIXME Stefan: when?
tho still supported, and the
new format is only recognized in some cases; more specifically the new
unquote was only recognized within a new backquote, and the new
backquote was only recognized if it occurs within a new backquote or
if it does not immediately follow an open parenthesis.
...
\begin{verbatim}
  (let ((buffer-file-name "/home/rms/.emacs"))
    ...
    ...)
"/afs/deinprogramm.de/home/sperber/data/papers/hopl-4-emacs-lisp/main.tex"\end{verbatim}
...
representation of the code.  In July 1985, Emacs-16.56 also came with
...
\item The available implementations of TCO affect the behavior not only in terms of reducing the stack
...
%% FIXME Mike: Maybe this should be moved to the sec:profiler section (which
...
started to lose momentum startomg about 2010.
2008 was also the year when Richard Stallman stepped down (again)
...
Emacs.  The main resistance from Stallman was coming from the last point
