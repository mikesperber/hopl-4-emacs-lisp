...
In Common Lisp, special forms accepting a place can obtain five values 
...
The original \texttt{cl.el} code followed the Common Lisp design. When
implementing the
...
So the reimplementation uses a different design: a \emph{place} 
maps to a single higher-order function (instead of a the
five values of the Common Lisp design).
...
%% FIXME: We should probably talk about memory management here as well,
...
The memory management code allocated objects by blocs of 4KB dedicated to
a particular kind of objects: one bloc each for cons cells, floats, symbols,
markers, and strings, all other objects were allocated directly with
\texttt{malloc}.  The ``mark'' bit in each 32bit box was not used for the
object contained in the 32bit box.  Instead it was used so that cons cells
could occupy only 2 words: the extra bit needed to store the mark&sweep's
...
Of course, these limits were still not actually comfortable.  So during the
development of Emacs-22, the tagging scheme was significantly reworked.
...
In Emacs-24.4, the representation of \texttt{Lisp\_Vector} objects (which is
used for many more object types than just vectors) was improved so as to
reduce their header from 2 words down to a single word.
