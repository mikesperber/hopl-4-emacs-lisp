...
through \textit{docstrings}, described in Section~\ref{sec:docstrings}.
...
\end{alltt}
%
This \textit{docstring} can be retrieved in various ways, in
particular through the user interface.  While MacLisp did not have
docstrings, notably Common Lisp~\cite{HyperSpec} also adopted them.  The exact
genealogy between the two is unclear.
...
file size.  The mark bits movd to the object headers, which were also
...
%% builds, is its performance competitive, known cases where it
%% shines, ...).
%% FIXME Stefan: better?
...
%% FIXME Stefan: Tried to clean up the wording.
...
Eventually howeevr such code would break in the face of representation
