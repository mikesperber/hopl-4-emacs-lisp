...
offered by Boehm's GC.
%% FIXME Stefan: Is it worth going through the details of hash-tables's weakness
...
% FIXME Stefan: can we have an example?
...
% FIXME Stefan: Some other sections mention "new cl.el by David
% Gillespie", but this isn't mentioned here.  It should.
...
% FIXME Stefan: This needs examples for setf use here
...
implementation were:
\begin{itemize}
\item The implementor of \texttt{cl-lib} % FIXME Stefan: Who?
...
Lisp, however, was not not motivated by immediate requirements, but
rather framed by Stallman's environment and experience at the time. As
a result, the original \Elisp{} as the simplest it could be while
still avoiding the pitfalls of TECO.

Being a Lisp has meant that the core language did not have to change
significantly since its original inception.  What would have been a
...
With Stefan Monnier taking over maintainership in 2008, language
design received more attention as goal in itself.  The language
designer's perspective has driven changes such as the introduction of
lexical scope, features from Common Lisp, and \texttt{pcase}.
