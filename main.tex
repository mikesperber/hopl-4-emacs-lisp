...
%% FIXME Mike: *mostly* but not completely (e.g. not true at least for Gerd Möllmann
%% and Joe, probably others)
...
without merging them.  Richard Stallman felt~\cite{RMS-text-props} like
...
incompatibilities, so the change lived in a separate branch for 4 years
...
packages, including within Emacs's own packages.  For some packages, it is
...
the Emacspeak package which advises hundreds of \Elisp{} functions to
...
a discussion with a user querying how to ``put several functions into
...
custom data type definitions for representing \Elisp{} entities, which
