...
used very widely and removing it would have broken many third-party packages
still in use, yet lacking any maintainer.  Over the years, these packages
have been slowly updated, so this syntax is expected to be finally
removed soon.
...
stack scanning (see Sec.\ref{sec:stack-scanning}) keeps track of every part
of the Lisp
...
This time the motivation was just to simplify the code, tho it came with
...
generic function \texttt{point-distance} which is executed after the other
methods of that generic function, but only when both
arguments are subtypes of \texttt{point3d}:
\begin{verbatim}
    (defclass point3d (point2d)
      (z :documentation "Third dimension"))
    (defmethod point-distance :after ((p1 point3d) (p2 point3d))
      (message "We just compared two point3d objects!"))
\end{verbatim}
...
of the same problems as \texttt{cl.el} in that it is not
``namespace clean''.
