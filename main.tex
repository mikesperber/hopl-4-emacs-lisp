...
demand.
%% FIXME Mike: This makes it sound like after those "many years" XEmacs
%% developers finally did change the bytecode format, yet we don't talk
%% about that change.
For many years, the XEmacs developers avoided changing the
byte-code format to make byte-code files interchangeable between Emacs
and XEmacs..
...
So Stefan Monnier developed instead the new macro \texttt{define-inline}
macro, which was included in Emacs-25.1.  It lets one define a function in
such a way that from its definition the macro can extract both a normal
function body and a corresponding compiler-macro.
