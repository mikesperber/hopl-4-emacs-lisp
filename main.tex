...
Richard Stallman evolved into the first version of Emacs~\cite{Seibel2009}.
...
Lisp as their extension language.  Richard Stallman, who had worked on
...
implementation language as well.  But high-performance Lisp compilers were not
...
and preserved in the history books as \emph{Gosling Emacs}, satisfied some
of those design goals: it was implemented in C and it featured an extension
...
Richard Stallman started Emacs as we know it today in late 1984 by replacing
...
particularly strong because of Richard's role in Emacs.
...
The Mock Lisp primitives differed from those of ZWEI in that ZWEI primitives
would always take arguments to specify what text to operate on, whereas in
Mock Lisp primitives all operated at the current \emph{point} (the current
``cursor position''), as was the case in TECO.  Stallman, who had worked on
...
%% highlights the differences and similarity mentioned above?
...
started right from the beginning as a real programming language with
...
the \Elisp{} language.  Retrospectively, to the best of our knowledge, those
...
variant of MacLisp~\cite{Moon1974,Pitman1983}, laced with a bit of Lisp
Machine Lisp, and put on a diet to keep it lean enough for use on the
limited resources of the machines of the time.
...
\Elisp{} also supports arrays, but using subset (e.g. only uni-dimensional
...
approach of manipulating arrays as if they were functions.

One of the most famous (mis)features of \Elisp{} is that it only offered
dynamic scoping of variables, where the latest binding of a variable is
...
\texttt{set} function, which performed assignment, accepted the \emph{name}
of a variable as its argument.  A symbol had an associated \emph{property
list}, essentially a dictionary mapping property names (also symbols)
to values.

Like MacLisp, \Elisp{} was (and still is) a Lisp-2
