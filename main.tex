...
Emacs's original inception was as a set of macros and keybindings for
the TECO editor.  TECO was written by Richard Stallman, then at the
MIT AI lab, and featured a primitive programming
language~\cite{Stallman2018-personal}.  TECO had a ``real-time edit
mode'' (then a novelty), which updated the text being edited as typing
happened.  This mode allowed mapping characters to commands, and users
would typically configure TECO to bind printing characters to
self-insert, and control chracters would execute TECO macros.  Guy
Steele coordinated among the users, creating a common set of
keybindings, which Richard Stallman evolved into the first version of
Emacs~\cite{Seibel2009}.  Note that TECO and Emacs were not completely
separate entities in the sense that TECO's extension language was also
Emacs's extension language and that they evolved together, where
TECO's extension language was regularly extended to provide facilities
then used by Emacs~\cite{Stallman2002}.
